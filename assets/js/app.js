        ...
        controller: function ($scope, $element, $attrs) {
            ...
            $scope.loadManifest = function (id) {
                ...
                socket.get('/manifest', {
                    ...
                    if ($scope.activeManifest.assignedUser) {
                        socket.get("/user", {
                            id: $scope.activeManifest.assignedUser
                        }, function (resp) {
                            $scope.activeManifestUser = resp;
                            $scope.$apply();
                        });
    ...
    return {
        ...
        link: function ($scope, iElm, iAttrs, controller) {
            ...
            function documentByManifest() {
                ...
                socket.get("/manifest/documents_by_manifest", {
                    manifestid: $scope.activeManifest.id
                }, function (resp) {
...
UCMDocument.directive('ucmDocumentManager', ['$http', function () {
    // Runs during compile
    return {
        // name: '',
        // priority: 1,
        ...
        controller: function ($scope, $element, $attrs) {
            ...
            //an array of all the currently selected documents
            $scope.selectedItems = [];
            //placeholder for the selected manifest
            $scope.selectedManifest="";


            /**
             * reloads the files based on the dir
             * @param dir
             */
            $scope.loadNewDirectory = function () {
                socket.post("/Document/listDocumentService", {
                    dir: $scope.baseUrl
                }, function (resp) {
                    $scope.docs = [];
                    $scope.$apply();
            ...
            //watch the base url and if changed refresh listing
            $scope.$watch('baseUrl', function (newValue, oldValue) {
               $scope.loadNewDirectory()
            });
            //initialise the root directory
            $scope.baseUrl = "C:/Users/mlawrence/Desktop/test";
...
UCMDocument.directive('ucmBreadcrumbs', ['$http', function(){
    // Runs during compile
    return {
        // name: '',
        // priority: 1,
