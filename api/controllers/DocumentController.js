...
//        console.log(ROOT_DIR.slice(0,1));
        ...
        fs.readdir(ROOT_DIR, function (err, files) {
            ...
            if (err) {
                ...
                var socket_msg = {action: "error", msg:"You cannot access this directory"};
...
//                console.log("x", files[i].startsWith("."));
...
//                                console.log("geting stats for: " + doc.filePath);
                                        ...
                                        if (stats && stats.isFile()) {
                                            ...
                                            Document.update(doc.id, {isFile: true, stats: stats}, function (err, udoc) {
//                                        console.log(udoc[0].filePath);
                                                ...
                                                var child = exec(sails.config.ucmImporter.JAVA_HOME + ' -jar ' + sails.config.ucmImporter.APACHE_TIKA_HOME + ' -v -j "' + udoc[0].filePath + '"', function (error, stdout, stderr) {
                                                    ...
                                                    if (error) {
                                                        ...
                                                        Document.update(doc.id, {metadata: JSON.parse(stdout)}, function (err, udoc) {
                                ...
                                //req.socket()
                                var socket_msg = {type: "Document", id: doc[0].id, action: "exists"};
//                        Document.publish(req, socket_msg);
                                sails.io.sockets.socket(req.socket.id).emit('message', socket_msg);
